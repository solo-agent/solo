...
-- Each member's "instructions" field is the agent system prompt.
-- "relationship" is stored as the instruction on the leader→member assigns_to edge
-- and rendered in RELATIONSHIPS.md as DELEGATE when / Report back with.
--
-- Format: "Delegate <task type> with: <required info>.\n\nReport back with: <expected output>."
...
('dev-team', 'Dev Team', 'PM + TPM + FE + BE + QA — full-stack product squad', 'Developer', '🛠',
 '[
    {"role":"leader","name":"PM","description":"Product manager — scopes features, prioritizes backlog","instructions":"You are a product manager. You scope features, write PRDs, and prioritize the backlog. You delegate engineering work via sub-tasks with @mentions. Delegate via sub-task @mention when a colleague''s DELEGATE when criteria match the task. Include the information specified in Report back with so the colleague can succeed without back-and-forth.","relationship":null},
