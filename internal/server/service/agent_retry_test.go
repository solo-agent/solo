...
func TestConfigurationFailureReturnsTaskToCreatorOnce(t *testing.T) {
	pool := agentRunTestPool(t)
	ctx := context.Background()
	ownerID := agentRunUser(t, pool)
	agentID := agentRunAgent(t, pool, ownerID)
	...
	_, err := pool.Exec(ctx, `
		INSERT INTO tasks (id, task_number, channel_id, creator_id, title, status, claimer_id)
		VALUES ($1, 2, $2, $3, 'configuration failure', $4, $5)`,
		taskID, channelID, ownerID, TaskStatusInProgress, agentID,
	)
	if err != nil {
		t.Fatalf("insert task: %v", err)
	...
	run, err := runSvc.StartRun(ctx, StartRunInput{
		...
		Status: AgentRunStatusRunning, ActivityText: agentActivityAccepted,
	})
	if err != nil {
		t.Fatalf("StartRun: %v", err)
	}
	if err := runSvc.LinkTask(ctx, LinkRunTaskInput{RunID: run.ID, TaskID: taskID, Role: AgentRunTaskRolePrimary, Confidence: 1}); err != nil {
		t.Fatalf("LinkTask: %v", err)
	}
	if _, err := runSvc.AppendEvent(ctx, AppendRunEventInput{RunID: run.ID, Type: AgentRunEventRunStarted, Payload: map[string]any{
		"result_contract": agentResultContractVisibleMessage,
	}}); err != nil {
		t.Fatalf("append run_started: %v", err)
	}
	if _, err := runSvc.FinishRun(ctx, FinishRunInput{RunID: run.ID, Status: AgentRunStatusFailed, ActivityText: agentActivityFailed}); err != nil {
		t.Fatalf("FinishRun: %v", err)
	}
	if _, err := runSvc.AppendEvent(ctx, AppendRunEventInput{RunID: run.ID, Type: AgentRunEventError, Payload: map[string]any{
		"failure_code": agentFailureConfiguration, "retryable": false,
	}}); err != nil {
		t.Fatalf("append error: %v", err)
	}
	...
	var status, claimerID string
	if err := pool.QueryRow(ctx, `SELECT status, COALESCE(claimer_id::text, '') FROM tasks WHERE id = $1`, taskID).Scan(&status, &claimerID); err != nil {
		t.Fatalf("query task: %v", err)
	}
	if status != TaskStatusTodo || claimerID != "" {
		t.Fatalf("task = status %q claimer %q, want todo/unclaimed", status, claimerID)
	...
	var messageCount, eventCount int
	if err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM messages WHERE metadata->>'task_id' = $1`, taskID).Scan(&messageCount); err != nil {
		t.Fatalf("count messages: %v", err)
	}
	if err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM agent_run_events WHERE run_id = $1 AND type = $2`, run.ID, AgentRunEventTaskRecoveryBlocked).Scan(&eventCount); err != nil {
		t.Fatalf("count events: %v", err)
	}
	if messageCount != 1 || eventCount != 1 {
		t.Fatalf("blocked artifacts = messages %d events %d, want 1/1", messageCount, eventCount)
	}
}

func TestAutomaticTaskRecoveryMode(t *testing.T) {
	mode, ok := automaticTaskRecoveryMode(retryableTaskRun{FailureCode: agentFailureProviderTransient, Retryable: true})
	if !ok || mode != taskRecoveryModeResumeSession {
		t.Fatalf("provider recovery = %q/%t", mode, ok)
	}
	mode, ok = automaticTaskRecoveryMode(retryableTaskRun{FailureCode: agentFailureMissingVisibleResult, Retryable: true})
