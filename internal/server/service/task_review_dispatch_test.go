...
func TestTaskReviewPromptBindsCurrentTaskAndImmutableEvidence(t *testing.T) {
	...
	for _, required := range []string{"[target=channel-id:task-message-id msg=task-message-id type=system]", "solo message send --target 'channel-id:task-message-id'", "solo task evidence -n 7 -c channel-id --submission submission-id", "reproducible input", `"artifact_version":"actual-digest"`} {
