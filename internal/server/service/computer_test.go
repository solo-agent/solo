...
func TestMarkConnectedKeepsLegacyDaemonIdentityReusable(t *testing.T) {
	...
	legacyID := uuid.NewString()
	if _, err := pool.Exec(ctx,
		`INSERT INTO computers (id, name, daemon_id, status, os, hostname)
		 VALUES ($1, 'legacy-mac', 'daemon-test-reusable', 'online', 'darwin', 'reusable-mac')`,
		legacyID,
	); err != nil {
		t.Fatal(err)
	}
	...
	if err := svc.MarkConnected(ctx, paired.Computer.ID, "daemon-test-reusable", "test", ComputerProtocolVersion, nil, ComputerSystemInfo{OS: "darwin", Hostname: "reusable-mac"}, nil); err != nil {
		t.Fatal(err)
	}
	var daemonID, status string
