...
func renderArtifactAgentPrompt(data artifactRenderData, mode string) string {
	...
	b.WriteString("Do not guess from old workspace files, do not pick files by newest mtime, and do not replace a real deliverable with a conversation summary. If there is no explicit current deliverable, use the `solo-artifacts` skill from this workspace with Solo's default Warm Archive skin: pick the artifact type, read the matching `references/<type>.md`, assemble from `assets/starter.html`, inline `assets/base.css`, and inline the needed `assets/interactions.js` modules.\n\n")
...
func renderPendingArtifactHTML(data artifactRenderData) string {
	...
	b.WriteString("body{font-family:ui-sans-serif,system-ui;margin:0;background:oklch(0.975 0.008 80);color:#39342f}main{max-width:760px;margin:0 auto;padding:48px 24px}.panel{background:oklch(0.995 0.003 80);border:1px solid oklch(0.865 0.014 75);border-radius:12px;box-shadow:0 12px 30px rgb(57 52 47 / 15%);padding:24px}h1{font-family:Georgia,serif;font-size:28px;margin:0 0 12px;font-weight:700;letter-spacing:-.015em}.badge{display:inline-block;border:1px solid oklch(0.865 0.014 75);border-radius:999px;background:#dbe5ed;padding:4px 8px;font-size:12px;font-weight:700;text-transform:uppercase}.meta{display:flex;gap:8px;flex-wrap:wrap;margin:16px 0}p{line-height:1.55}code{background:oklch(0.955 0.01 80);border-radius:4px;padding:2px 4px}footer{margin-top:20px;color:#6e665e;font-size:12px}")
	...
	b.WriteString("<p>The leader agent is building the Warm Archive artifact and will publish it back into Solo with <code>solo artifact publish</code>.</p>")
