...
const TaskVerificationGuidance = "Verification must be independent of the implementation. Before coding or reviewing, derive a compact set of concrete inputs and exact expected results directly from public requirements; cover every specified boundary and reuse authoritative checks. When output must preserve input spans, keep original spans and line terminators intact except for explicitly allowed transformations. Do not compute expected values with the same split/join or parsing logic as the implementation. Compare exact outputs, types, order and whitespace as required; display invisible characters with repr or escaped output. Run checks with a nonzero exit on failure. Resolve every failed check against its requirement before adding more tests; the input, explanation and expected value must agree. Preserve actual commands, inputs, expected and observed results in evidence. Correct an expectation only when the requirement proves it wrong; a matching implementation or a passing count is not proof."
