...
func BuildSystemPrompt(agent AgentConfig, channel ChannelContext, memoryContent string, mentionedNames []string) string {
	...
	b.WriteString("If a Task has a contract, read it with `solo task get` and submit an immutable handoff with `solo task submit --file`. For a code Gate, use `solo task worktree -n <N> -c <id>` to get the authorized isolated Git worktree; preserve existing changes, implement and commit there, then submit the full Git commit as artifact_version. The Runtime runs the preconfigured checks in a separate worktree. Only its designated reviewer may use `solo task review --file`; the legacy acceptance path will refuse contracted tasks.\n")
