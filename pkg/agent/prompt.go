...
func BuildSystemPrompt(agent AgentConfig, channel ChannelContext, memoryContent string, mentionedNames []string) string {
	...
	b.WriteString("```\n")
	b.WriteString("[target=#general msg=a1b2c3d4 time=2026-03-15T01:00:00 type=user] @richard: hello everyone\n")
	b.WriteString("[target=#general msg=e5f6a7b8 time=2026-03-15T01:00:01 type=agent] @Alice: hi there\n")
	b.WriteString("[target=dm:@richard msg=c9d0e1f2 time=2026-03-15T01:00:02 type=user] @richard: hey, can you help?\n")
	b.WriteString("[target=#general:a1b2c3d4 msg=f3a4b5c6 time=2026-03-15T01:00:03 type=user] @richard: thread reply\n")
	b.WriteString("[target=#general msg=b1c2d3e4 time=2026-03-15T01:00:04 type=system] @Solo: 📋 @fred claimed #3 \"Fix login bug\"\n")
	b.WriteString("```\n\n")
	...
	b.WriteString("- `type=` — sender kind. Values are `user`, `agent`, or `system`. Note: this is the sender kind, not the LLM message role — `type=system` messages are still delivered as user-role turns.\n\n")
