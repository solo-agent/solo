...
const TaskVerificationGuidance = "Verification must be independent of the implementation. Before coding or reviewing, derive a compact set of concrete inputs and exact expected results directly from public requirements; cover every specified boundary and reuse authoritative checks. When output must preserve input spans, keep original spans and line terminators intact except for explicitly allowed transformations. Include an executable reconstruction check: retained outputs plus exactly the removed spans must reproduce the normalized input. Do not compute expected values with the same split/join or parsing logic as the implementation. Compare exact outputs, types, order and whitespace as required; display invisible characters with repr or escaped output. Run checks with a nonzero exit on failure. Resolve every failed check against its requirement before adding more tests; the input, explanation and expected value must agree. Preserve actual commands, inputs, expected and observed results in evidence. Correct an expectation only when the requirement proves it wrong; a matching implementation or a passing count is not proof."
...
func BuildSystemPrompt(agent AgentConfig, channel ChannelContext, memoryContent string, mentionedNames []string) string {
	...
	if agent.WorkspacePath != "" {
		fmt.Fprintf(&b, "- Workspace (current working directory): %s\n", agent.WorkspacePath)
	}
	if agent.SkillsDigest != "" {
		fmt.Fprintf(&b, "- Versioned Skills digest: %s. Read the applicable SKILL.md and its resources under .solo/revision-skills/current/<skill-name>/ in your workspace. These files are pinned to this Run's Revision; propose edits as a new Revision rather than editing the installed snapshot.\n", agent.SkillsDigest)
	}
	if agent.Name != "" {
	...
	b.WriteString("After a freshness hold, read all returned updates before deciding. You may revise the draft, explicitly retain it with `solo message send --keep-after-seq <latest seen sequence> --freshness-reason <reason>`, or discard the exact draft using `solo work list` then `solo work discard --file <JSON with run_id, sha256, reason>`. Retaining still checks for new messages; discarding does not complete a Task or work mark.\n\n")
	...
	b.WriteString("**Contract submission JSON:**\n```json\n{\"expected_task_version\":1,\"idempotency_key\":\"delivery-1\",\"artifact_version\":\"<actual artifact digest or commit>\",\"handoff\":{\"summary\":\"<result>\",\"changes\":\"<changes>\",\"risks\":\"<remaining risks>\",\"next_steps\":\"<reviewer action>\"},\"evidence\":[{\"id\":\"E1\",\"description\":\"Actual verification\",\"content\":\"<actual command, input and output>\"}]}\n```\nReplace the example values with the latest Task version and your actual results. Every evidence item needs a unique id, a description and content or a URI with SHA256. Reuse the same idempotency key only for an identical retry. Do not invent evidence or search another Agent's workspace for submission examples.\n")
	b.WriteString("Propose reusable improvements with `solo work propose-revision --file <JSON>` (summary, config: system_prompt/model_provider/model_name/custom_args, optional skills). Each Skill has name and files [{path,content(base64),executable}]; include root SKILL.md and only explicitly reusable resources. Never copy private Memory, conversations, credentials, or a whole workspace. `solo work revisions` lists immutable proposals; only the human owner can publish after evaluation.\n")
	b.WriteString("After proposing an improvement supported by actual feedback, prepare the fixed regression cases yourself and start it with `solo work start-selection --file <JSON>`: channel_id (the explicit team whose work should improve), candidate_revision_id, receiver_agent_id (another owned member of that team), reviewer_agent_id (a different owned member from both yourself and receiver), problem, change, cases [{title,input,requirements:[{id,text}]}], team_check, token_budget (per execution/review member), idempotency_key. Use only your authorized Computer and budget. Reuse the same key for an identical retry. The Server creates isolated fixed revisions, runs the three evaluation arms and dispatches an independent reviewer automatically. Read `solo work selections` at a meaningful next opportunity; do not poll or ask the human to click every case. Missing reviewer, permissions or budget require a concise specific request. Do not create repeated comparisons without new evidence. Only present adoption after exact candidate and receiver results are verified; only the owner may decide and apply.\n")
	b.WriteString("At the beginning of assigned work and before reporting completion, use `solo work list` to recover pending commitments across Sessions. Update completed or cancelled commitments with concrete resolution; handle obsolete held drafts yourself. Do not ask the user to operate work marks or draft-discard controls. Record an obligation that is not yet a Task with `solo work mark --file` (channel_id, description, next_action, idempotency_key, optional source_message_id/task_id). Resolve it explicitly with id, status=resolved or cancelled, and resolution. Reading a message or finishing a Run does not resolve a commitment. `solo work attention --file` sets policy=all/mentions/nothing with optional channel_id.\n")
	...
	b.WriteString("- Without `--assignee`, `solo task create` only creates the task; use `solo task claim` to own it. With `--assignee <agent>`, creation atomically assigns and wakes that Agent.\n")
	b.WriteString("- Typical uses are delegating a subtask to a specific colleague or creating genuinely new work for normal claiming.\n")
	...
	b.WriteString("A folder path or repository URL mentioned in a message is task context, not a persistent Channel working-directory binding. Inspect or use it only when the user asks and it is available. If it becomes a recurring resource, keep a checkout or worktree inside your Agent workspace and record the durable reference in MEMORY.md or its note index. Do not claim to have inspected a resource until you have actually obtained it.\n\n")
