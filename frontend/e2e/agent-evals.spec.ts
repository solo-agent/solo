    ...
    for (const c of cases) for (let repetition = 0; repetition < repetitions; repetition++) {
      ...
      for (const strategy of ordered) {
        ...
        try {
          ...
          expect(sql(`SELECT (extract(epoch FROM due_date)*1000)::bigint FROM tasks WHERE id=${quote(task.id)}`)).toBe(String(trialDeadline));
