...
test('real author submits evidence; human and independent Agent accept exact submissions', async ({ page, request }, testInfo) => {
  ...
  await expect.poll(() => sql(`SELECT count(*) FROM agent_runs WHERE agent_id='${author.id}' AND status='completed'`), { timeout: 240_000 }).toBe('1');
    ...
    // Task acceptance precedes the reviewer's final message. Cleanup must wait
    // for every real Run and its usage, or it can cancel a successful review.
    await expect.poll(() => sql(`SELECT count(*) FROM agent_runs r LEFT JOIN agent_run_token_usage u ON u.run_id=r.id WHERE r.agent_id IN (${agents.map(id => `'${id}'`).join(',')}) AND (r.status<>'completed' OR r.finished_at IS NULL OR u.actual_tokens IS NULL)`), { timeout: 60_000 }).toBe('0');
    expect(Number(sql(`SELECT count(*) FROM messages m JOIN threads th ON th.id=m.thread_id JOIN tasks t ON t.message_id=th.root_message_id JOIN agent_runs r ON m.metadata->>'agent_run_id'=r.id::text WHERE t.id='${pinnedTask.id}' AND m.sender_id='${reviewer.id}' AND r.agent_id=m.sender_id AND r.status='completed'`))).toBeGreaterThan(0);
