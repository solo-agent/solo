  ...
  return (
    <section data-testid="automation-workspace" className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
      <div className="mb-4 flex items-start justify-between gap-4 border-b-2 border-black pb-4">
        ...
        {!editing && !historyFor && <Button type="button" variant="primary" size="sm" onClick={startCreate}><Plus className="mr-1.5 h-4 w-4" />{t('automationNew')}</Button>}
      </div>
      ...
      {editing ? (
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="button" variant="primary" onClick={() => void save()} disabled={saving}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{editing === 'new' ? t('automationCreate') : t('automationUpdate')}</Button>
          </div>
        </div>
      ) : historyFor ? (
