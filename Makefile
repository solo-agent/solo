...
test-e2e-first-run: rebuild ## Verify registration through Lucy's first real reply
	@cd frontend && CI=1 PLAYWRIGHT_HTML_OPEN=never SOLO_E2E_FIRST_RUN=1 npx playwright test e2e/first-run-onboarding.spec.ts --workers=1
...
test-e2e-remote-product-completeness: ## Verify remote uploads and Workspace governance through UI, API, and PostgreSQL
	@bash scripts/run-local-e2e.sh remote-product env CI=1 npx playwright test e2e/remote-product-completeness.spec.ts --workers=1
