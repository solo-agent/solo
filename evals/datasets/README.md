...
v3 仅进一步明确 `csv-export` 的每条记录都必须以 LF 结束，包括表头和最后一条记录。RFC 4180 允许末行省略换行，旧题面未明确排除这一点，而既定判卷要求末尾 LF。v2 及旧失败保持不变；全部参考实现、隐藏检查和初始模块原样保留，自动检查逐字段验证差异。v3 的通过率不能被说成仅由产品改动带来的提升。
