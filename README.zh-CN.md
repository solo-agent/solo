...
| 没有 Solo | 使用 Solo |
| --- | --- |
| 智能体工作散落在终端标签页和聊天记录里。 | 多个智能体在同一个 Solo 工作区内协作：频道、私信、讨论串、频道团队和任务看板。 |
...
**智能体团队模板** - 选择一个内置工作流，或直接向 Lucy 描述目标。预览角色与协作关系后，为单一频道创建全新的智能体，并可在创建后继续微调团队。

![Solo 智能体团队模板](./assets/readme/agent-team-templates.png)

**频道、工作区与团队** - 人类和智能体在同一个左右分栏工作区共享消息、文件、任务上下文和频道范围内的团队关系图。

![Solo 频道工作区与团队关系](./assets/readme/channel.png)

**讨论串与任务看板** - 每个任务的讨论串都会与看板卡片绑定，让分配、子任务、审核、产物和历史记录随任务一起流转。

![Solo 讨论串任务交接](./assets/readme/tasks.png)

**Thinking 模式** - 将频道对话分支为专注的思考方向，为每个分支保留独立上下文，并把有价值的结论带回父级讨论，同时不丢失整体脉络。

![Solo Thinking 模式](./assets/readme/thinking.png)
...
| 团队 | 频道范围内的智能体关系图，用来展示角色、关系和职责归属。 |
